            ...
            <div class="modal-content container h-auto text-left">
                ...
                <table class="text-sm border-collapse border-spacing-0">
                    ...
                    <tr>
                        ...
                        {{-- Optimized: Direct array iteration instead of collection implode --}}
                        @foreach($this->getSpecimenIds() as $specimenId)
                            <th scope="col"
                                class="text-center border-b border-gray-500 px-2 z-20 sticky bg-gray-200 top-0">
                                {!! $this->getDateTimeHeader($specimenId) !!}
                            </th>
                        @endforeach
                    </tr>
                    </thead>
                    @if($labs && count($labLookup) > 0)
                        <tbody>
                        @php($loopPanel = '')
                        {{-- Optimized: Direct array iteration instead of collection foreach --}}
                        @foreach($labLookup as $labName => $labLabel)
                            <tr class="border-b border-gray-500 hover:bg-sky-200 group">
                                @if ($labLabel['panel'] != $loopPanel)
                                    <th rowspan="{{ $this->getPanelCount($labLabel['panel']) }}"
                         ...
                         z-20 sticky left-7">{{ $labLabel['label'] }}</th>

                                {{-- Optimized: O(1) lookup instead of O(n*m) collection operations --}}
                                @foreach($this->getSpecimenIds() as $specimenId)
                                    @php($lab = $this->getLabForSpecimen($specimenId, $labName))
                                    <td class="border-r border-gray-500 px-2 text-center whitespace-nowrap {{ $this->isLabFlagged($lab) }}">
                                        {{ $lab['result'] ?? '' }}
                                    </td>
                                @endforeach
                            </tr>
                ...
                {{-- Micro Results Table (keeping original structure for compatibility) --}}
                <table class="text-sm border-collapse border-spacing-0">
                    <thead>
                    <tr>
                ...
                {{-- Unparsable Rows Section --}}
                @if($labs && count($unparsableRows) > 0)
                    <div class="bg-white">
